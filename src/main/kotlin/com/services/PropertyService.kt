...
class PropertyService {
    ...
    suspend fun createProperty(request: CreatePropertyRequest, ownerId: String): PropertyResponseDto {
        val property = Property(
            ...
            specifications = request.specifications.mapNotNull { it },
            area = request.area,
            rooms = request.rooms,
            baths = request.baths,
            price = request.price,
    ...
    suspend fun updateProperty(id: String, request: UpdatePropertyRequest, ownerId: String): PropertyResponseDto {
    ...
    suspend fun deleteProperty(id: String, ownerId: String): Boolean {
        val existing = propertyRepository.findById(id)
            ?: throw NoSuchElementException("Property not found")
    ...
    suspend fun getUserProperties(ownerId: String, page: Int, limit: Int): Pair<List<PropertyResponseDto>, Long> {
    ...
    suspend fun updatePropertyStatus(id: String, status: String): Boolean {
        return propertyRepository.updateStatus(id, status)
    }

    suspend fun adminDeleteProperty(id: String): Boolean {
        val property = propertyRepository.findById(id)
        if (property != null) {
            // Decrement user's ad count
